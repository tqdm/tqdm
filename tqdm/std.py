...
from typing import IO, Any, AnyStr, Generator, Literal, overload
from warnings import warn
from weakref import WeakSet

from ._monitor import TMonitor
from .utils import (
    CallbackReadIOWrapper,
    CallbackWriteIOWrapper,
    Comparable,
    DisableOnWriteError,
    FormatReplace,
    SimpleTextIOWrapper,
...
def TRLock(*args: Any, **kwargs: Any):
    """threading RLock"""
    try:
...
class Bar:
    ...
    def colour(self, value):
        ...
        try:
            ...
            warn(
                "Unknown colour ({}); valid choices: [hex (#00ff00), {}]".format(
...
class tqdm(Comparable):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            ...
            if dynamic_ncols:
                dynamic_ncols = screen_shape_wrapper()
                if dynamic_ncols:
                    ncols, nrows = dynamic_ncols(file)
            else:
                _dynamic_ncols = screen_shape_wrapper()
                if _dynamic_ncols:
        ...
        if ascii is None:
            ascii = not supports_unicode(file)

        if bar_format and ascii is not True and not is_ascii(ascii):
    ...
    def wrapattr(
        ...
        bytes: bool = True,
        **tqdm_kwargs: Any,
    ) -> Generator[CallbackReadIOWrapper, None, None]: ...
        ...
        **tqdm_kwargs: Any,
        ...
        with cls(total=total, **tqdm_kwargs) as t:
            if bytes:
                ...
                t.unit_divisor = 1024
            if method == "write":
                yield CallbackWriteIOWrapper(t.update, stream)
            else:
                yield CallbackReadIOWrapper(t.update, stream)

    # leagcy

    status_printer = staticmethod(status_printer)
    format_num = staticmethod(format_num)
    format_meter = staticmethod(format_meter)
    format_interval = staticmethod(format_interval)
