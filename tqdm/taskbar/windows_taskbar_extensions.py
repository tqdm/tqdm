...
if sys.getwindowsversion() >= (6, 1):
    from ctypes import c_wchar_p, c_int, c_ulonglong, POINTER
    ...
    ITaskbarList3._methods_ = [
        COMMETHOD([], HRESULT, 'SetProgressValue', ( ['in'], c_int, 'hwnd' ), ( ['in'], c_ulonglong, 'ullCompleted' ), ( ['in'], c_ulonglong, 'ullTotal' )),
        COMMETHOD([], HRESULT, 'SetProgressState', ( ['in'], c_int, 'hwnd' ), ( ['in'], c_int, 'tbpFlags' )),
        dummyMethod,
        dummyMethod,
        dummyMethod,
        dummyMethod,
        ...
        dummyMethod,
        dummyMethod,
        COMMETHOD([], HRESULT, 'SetOverlayIcon', ( ['in'], c_int, 'hwnd' ), ( ['in'], POINTER(IUnknown), 'hIcon' ), ( ['in'], c_wchar_p, 'pszDescription' )),
        COMMETHOD([], HRESULT, 'SetThumbnailTooltip', ( ['in'], c_int, 'hwnd' ), ( ['in'], c_wchar_p, 'pszTip' )),
        dummyMethod,
    ]
    ...
    class WindowsTaskbarProgressReporter(PlatformSpecificProgressReporter):
        taskBarList=None
        ...
        def progress(self, current:int, speed:float):
            if self.total:
                self.__class__.taskBarList.SetProgressState(self.hwnd, TBPFlag.normal)
                self.__class__.taskBarList.SetProgressValue(self.hwnd, current, self.total)
                #self.__class__.taskBarList.SetThumbnailTooltip(self.hwnd, "tqdm: "+str(current)+"/"+str(total))
            else:
                self.__class__.taskBarList.SetProgressState(self.hwnd, TBPFlag.indeterminate)
                #self.__class__.taskBarList.SetThumbnailTooltip(self.hwnd, "tqdm: "+str(current))
        
        def fail(self, reason:str=None):
            self.__class__.taskBarList.SetProgressState(self.hwnd, TBPFlag.error)
        ...
        def success(self):
            ...
            self.__class__.taskBarList.SetProgressState(self.hwnd, TBPFlag.noProgress)
            pass
        ...
        def __enter__(self):
            if not self.__class__.taskBarListRefCount:
                self.__class__.taskBarList=comtypes.client.CreateObject('{56FDF344-FD6D-11d0-958A-006097C9A090}', interface=ITaskbarList3)
    ...
    from .PlatformSpecificProgressReporter import DummyProgressReporter as WindowsTaskbarProgressReporter
